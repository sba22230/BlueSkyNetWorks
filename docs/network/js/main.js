...
function initSigma(config) {
  ...
  dataReady = function () {
    ...
    // Replace the node iteration inside dataReady (initSigma)
    a.iterNodes(function (b) {
      // Use community attribute as the cluster key; fall back to color if missing
      var community =
        b.attr && b.attr.attributes && b.attr.attributes.community
          ? String(b.attr.attributes.community)
          : b.color;
      a.clusters[community] || (a.clusters[community] = []);
      a.clusters[community].push(b.id);
    });
    /*
		a.iterNodes(
...
function configSigmaElements(config) {
  ...
  // Replace the legend / group list generation in configSigmaElements
  var a = [],
    b,
    x = 1;
  for (b in sigInst.clusters) {
    // Use the first member's node color for the swatch
    var firstId = sigInst.clusters[b] && sigInst.clusters[b][0];
    var bg = "#ddd";
    if (
      firstId &&
      sigInst._core &&
      sigInst._core.graph &&
      sigInst._core.graph.nodesIndex[firstId]
    ) {
      bg = sigInst._core.graph.nodesIndex[firstId].color;
    }
    a.push(
      '<div style="line-height:12px"><a href="#' +
        ...
        bg +
        ';display:inline-block"></div> Community ' +
        b +
        " (" +
