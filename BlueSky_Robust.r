...
deep_search_posts <- function(query, hard_limit = 50000, chunk_limit = 100, checkpoint_path = "posts_checkpoint.parquet") {
  all_rows <- list()
  cursor <- NULL
  until <- NULL
  fetched <- 0
  ...
  while (fetched < hard_limit) {
    ...
    if (is.null(cursor)) {
      ...
      indexedAt_posix <- suppressWarnings(as.POSIXct(df$indexedAt, format = "%Y-%m-%dT%H:%M:%OSZ", tz = "UTC"))
...
hydrate_in_batches <- function(posts_df, batch_size = 400, tag = "speirgorm") {
  ...
  for (i in seq_along(rep_chunks)) {
    message(sprintf("Hydrating reposts batch %d/%d (%d uris)", i, length(rep_chunks), length(rep_chunks[[i]])))
    part <- future_map_dfr(rep_chunks[[i]], get_reposts_df, .progress = TRUE, .options = furrr_options(seed = 22230))
  ...
  for (i in seq_along(thr_chunks)) {
    message(sprintf("Hydrating threads batch %d/%d (%d uris)", i, length(thr_chunks), length(thr_chunks[[i]])))
    part <- future_map_dfr(thr_chunks[[i]], get_thread_df, .progress = TRUE, .options = furrr_options(seed = 22230))
  ...
  reposts_df <- bind_rows(all_reposts) %>% distinct(original_uri, handle, uri, .keep_all = TRUE)
  threads_df <- bind_rows(all_threads) %>% distinct(original_uri, author, uri, .keep_all = TRUE)
  
  list(reposts_df = reposts_df, threads_df = threads_df)
}

# Run deep search
posts_df <- deep_search_posts("Speirgorm", hard_limit = 50000, chunk_limit = 100, checkpoint_path = "speirgorm_posts.parquet")
...
edges <- edges %>%
  mutate(from = normalize_handle(from),
         to   = normalize_handle(to))
