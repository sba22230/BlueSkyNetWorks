...
```{r}
resp <- bs_search_posts("Speirgorm", clean = FALSE, limit = 100)
...
print(dplyr::select(posts_df1, uri, author_handle, indexedAt))
```
...
posts_df1$hashtags <- post_hashtags

## Quick check: show posts with hashtags
print(head(dplyr::select(posts_df1, uri, author_handle, hashtags)))
```
...
edges <- posts_df %>%
    filter(!is.na(post_reply_parent_uri_df)) %>%
