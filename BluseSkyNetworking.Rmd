isn--- title: "R Notebook" output: html_notebook ---
...
post_reply_parent_uri <- data.frame(uri = post_reply_parent_uri[!is.na(post_reply_parent_uri)])
...
get_reposts_df <- function(uri) {
  reposts <- bs_get_reposts(uri, clean = TRUE)
  if (length(reposts) == 0) return(NULL)
  
  # Convert to data frame and add original URI for reference
  data.frame(
    original_uri = uri,
    reposted_by = sapply(reposts, function(x) x$actor$handle),
    repost_uri = sapply(reposts, function(x) x$uri),
    stringsAsFactors = FALSE
  )
}
```

posts \<- post_reply_parent_uri\$uri %\>% map_dfr(get_reposts_df)
